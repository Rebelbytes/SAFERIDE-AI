...
def update_database_with_violation(plate_number, host, user, password, database):
    try:
        ...
        if connection.is_connected():
            ...
            if result:
                cursor.execute("UPDATE license_plates SET violation_count=violation_count+1 WHERE plate_number=%s", (plate_number,))
            else:
                cursor.execute("INSERT INTO license_plates (plate_number) VALUES (%s)", (plate_number,))
            connection.commit()
            cursor.close()
    except Error as e:
        print("Error:", e)
...
def draw_penalized_text(frame):
    ...
    y_pos += 80
    for text in penalized_texts:
        cv2.putText(frame, f'-> {text}', (40, y_pos), font, 1, (255, 255, 255), 2)
        y_pos += 60
